...
public class ReflectionUtils {
     ...
     * with {@link ReflectionUtils}. This method will substring it, for it to be eligible
     ...
     * @see {@link ParameterizedType#getActualTypeArguments()}
     */
    public static Type[] getParameterizedTypes4Class(Class object) {
        Type[] superclassType = object.getGenericInterfaces();
        if (!ParameterizedType.class.isAssignableFrom(superclassType[0].getClass())) {
            return null;
        }

        return ((ParameterizedType)superclassType[0]).getActualTypeArguments();
    }
    ...
    public static Type getGenericReturnType(Class<?> clazz, String name) {
        if (clazz==null || name==null || name.isEmpty()) {
            return null;
        }
     ...
     * @return
     */
    public static Type[] getGenericParamType(Class<?> clazz, String name) {
        if (clazz==null || name==null || name.isEmpty()) {
            return null;
        }
